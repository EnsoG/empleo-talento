...
async def login_oauth(
    ...
    try:
        ...
        if not user:
            ...
            user.last_connection = today
            session.add(user)
            await session.commit()
        # Define Data To Encode
        to_encode = {
            "sub": str(user.candidate_id if user else new_user.candidate_id),
            "email": email,
...
async def request_reset_password(
    ...
    try:
        ...
        if user_role == UserRoleEnum.admin:
            conditions.append(PasswordReset.admin_id == user.admin_id)
        ...
        password_reset = PasswordReset(
            ...
            admin_id=user.admin_id if user_role == UserRoleEnum.admin else None
        )
